...
from apps.atelier.scripts.components import carousel_swiper_smoke
...
def run():
    ...
    for _,modname,_ in pkgutil.iter_modules(importlib.import_module(package).__path__):
        ...
        if hasattr(m,"run"):
            ...
            results.append(res)
    print("→ Running carousel_swiper_smoke")
    res = carousel_swiper_smoke.run()
    if not isinstance(res, dict):
        res = {"ok": bool(res), "name": "carousel_swiper_smoke", "duration": 0.0, "logs": []}
    results.append(res)
