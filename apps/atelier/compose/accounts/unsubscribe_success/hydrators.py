...
def unsubscribe_success_message(request, params):
    data = params if isinstance(params, dict) else {}
    next_value = resolve_safe_next_url(request, data.get("next_value"))
    return {
        "title": data.get("title") or _("Désabonnement confirmé"),
        "description": data.get("description")
        or _("Tu ne recevras plus nos e-mails marketing. Tu peux te reconnecter pour mettre à jour tes préférences."),
