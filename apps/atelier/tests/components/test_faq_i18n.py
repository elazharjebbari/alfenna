...
class FaqI18NTests(TestCase):
    ...
    def test_first_question_translated_in_arabic(self) -> None:
        ...
        self.assertIn("هل تعالج هذه الحزمة الفطريات تماماً؟", html)
        self.assertIn("ضمان استرجاع خلال 30 يوماً", html)

    def test_first_question_in_french(self) -> None:
        request = self._request(namespace="core", lang="fr")
        page_ctx, slot_ctx = self._contexts("core")

        html = pipeline.render_slot_fragment(page_ctx, slot_ctx, request)["html"]

        self.assertIn("Est‑ce que ce pack <strong>guérit</strong> les mycoses ?", html)
        self.assertIn("Satisfait ou remboursé 30 jours.", html)
