...
def make_signed_token(body: dict, *, age: int = 0) -> str:
    ...
    payload = json.dumps(body_wo, sort_keys=True).encode("utf-8")
    msg = hashlib.md5(payload).hexdigest()
    secret = getattr(settings, "LEADS_SIGNING_SECRET", settings.SECRET_KEY)
    signature = hmac.new(secret.encode("utf-8"), f"{ts}.{msg}".encode("utf-8"), hashlib.sha256).hexdigest()
